...
# Try importing mpi4py, but provide fallback
try:
...
try:
    ...
    import os as bf
    print("Warning: blobfile not available. Using os module instead.")
...
def get_rank_and_world_size():
    """Helper function to get rank and world size with MPI fallback"""
    if MPI_AVAILABLE:
...
class CelebADataset(Dataset):
    def __init__(
        ...
        # partition_df = pd.read_csv(osp.join(data_dir, 'list_eval_partition-5000.csv'))
        partition_df = pd.read_csv('./list_eval_partition-5000.csv')
        self.data_dir = data_dir
        # data = pd.read_csv(osp.join(data_dir, 'list_attr_celeba-5000.csv'))
        data = pd.read_csv('./list_attr_celeba-5000.csv')
        if partition == 'train':
            partition = 0
        elif partition == 'val':
            partition = 1
    ...
    def __getitem__(self, idx):
        ...
        with open(osp.join(self.data_dir, img_file), "rb") as f:
            img = Image.open(f)
            img = img.convert('RGB')
...
class CelebAHQDataset(Dataset):
    def __init__(
        ...
        query_label=-1,
        normalize=True,
        start_index=9655,  # Add start_index parameter
    ):
        ...
        with open(('./CelebAMask-HQ/CelebA-HQ-to-CelebA-mapping.txt'), 'r') as f:
            mapstr = f.read()
        ...
        partition_df = pd.read_csv('~/ACE/list_eval_partition.csv')
        ...
        indices_to_keep = partition_df['partition'].index.intersection(data.index)
        self.data = data.loc[indices_to_keep]

        self.data = self.data[shard::num_shards]
        self.data.reset_index(inplace=True)
        self.data.replace(-1, 0, inplace=True)

        # Filter data to start from start_index
    ...
    def __getitem__(self, idx):
        ...
        with open(osp.join('./CelebAMask-HQ/CelebA-HQ-img', img_file), "rb") as f:
            img = Image.open(f)
            img = img.convert('RGB')
