    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                with bf.BlobFile(bf.join('./ddpm_cas2', filename), "wb") as f:
                    th.save(state_dict, f)

        # delete old checkpoints
        if dist.get_rank() == 0:
            for f in glob.glob(os.path.join('./ddpm_cas2', '*.pt')):
        ...
        if dist.get_rank() == 0:
            with bf.BlobFile(
                bf.join('./ddpm_cas2', f"opt{(self.step+self.resume_step):06d}.pt"),
                "wb",
            ) as f:
