    ...
    def forward(self, x):
        ...
        for idx, t in enumerate(timesteps):
            ...
            out = self.diffusion.p_mean_variance(
                self.ddpm, x, t,
                clip_denoised=True)
            # ce_np = out.cpu().detach().numpy()
            # np.save(f'{idx}_out.npy', ce_np)
            x = out['mean']
...
class Attack():
    ...
    def extract_grads(self, x, y):
        ...
        # ce_np1 = x.cpu().detach().numpy()
        # np.save(f'x_0.npy', ce_np1)
...
class ClassifierDiffusionCheckpointGradients(Attack):
        ...
        while True:
            ...
            for jdx, t in enumerate(diff_steps, start=idx + 1):
                ...
                if t == 'c':  # classification step, always final step
                    ...
                    ce_np1 = output.cpu().detach().numpy()
                    np.save(f'{jdx}_x.npy', ce_np1)
    ...
    def forward(self, x, t, idx, noise):
        ...
        ce_np1 = x.cpu().detach().numpy()
        np.save(f'{idx}_output.npy', ce_np1)
        return x
